...
void updateMovement(struct properties *p){
    ...
    if(p->revolver.stat & 0x80){

        if(p->revolver.stat & 0xF){
            if(p->revolver.stat&0x40)
                setMotors(1,1,255);
            else
                setMotors(1,0,255);
            if((p->time-p->revolver.time1 > 80)&&(p->revolver.stat&0x20)){
